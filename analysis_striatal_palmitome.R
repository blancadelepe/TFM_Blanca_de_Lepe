...
# independently 
iBAQ.K = intensity.raw.data[,1:3]; iBAQ.W = intensity.raw.data[,4:6]

# The rows of each group in which more than 1 NA appears are deleted.
iBAQ.K = iBAQ.K[rowSums(is.na(iBAQ.K)) < 2, ]
iBAQ.W = iBAQ.W[rowSums(is.na(iBAQ.W)) < 2, ]

iBAQ.K$mean.expr <- rowMeans(iBAQ.K, na.rm = T)
for (i in 1:nrow(iBAQ.K)){
  for (j in 1:ncol(iBAQ.K)){
    if (is.na(iBAQ.K[i,j])==TRUE){
      iBAQ.K[i,j] <- iBAQ.K[i,4]
    }
  }
}

iBAQ.W$mean.expr <- rowMeans(iBAQ.W, na.rm = T)
for (i in 1:nrow(iBAQ.W)){
  for (j in 1:ncol(iBAQ.W)){
    if (is.na(iBAQ.W[i,j])==TRUE){
      iBAQ.W[i,j] <- iBAQ.W[i,4]
    }
  }
}


interseccion = intersect(rownames(iBAQ.K[,1:3]),rownames(iBAQ.W[,1:3]))
...
fviz_pca_ind(res.pca, col.ind = c("iBAQ.K1","iBAQ.K2","iBAQ.K3","iBAQ.W1","iBAQ.W2","iBAQ.W3"), 
